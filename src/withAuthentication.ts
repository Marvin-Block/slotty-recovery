    ...
    try {
        ...
        if (user.role !== null && user.role !== "free" && user.expiry !== null && new Date(user.expiry) < new Date(Date.now())) {
            ...
            await Email.send({
                ...
                text: `Hello ${user.username}, Your ${user.role.charAt(0).toUpperCase() + user.role.slice(1)} subscription has expired. Please renew your subscription to continue using SlottyRecovery.`,
                html: `
                    <!DOCTYPE html>
                    <html>
                        <head>
