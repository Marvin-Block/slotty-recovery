    ...
    return new Promise(async resolve => {
        ...
        case "DELETE":
            try {
                ...
                if (servers.length > 0)
                    return res.status(400).json({ success: false, message: "Please Delete the following servers on SlottyRecovery dashboard, then try again", servers: servers.map((s) => s.id) });
