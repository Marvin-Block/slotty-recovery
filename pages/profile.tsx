...
export default function Verify({ info, servers, err }: { info: UserInfo, servers: Server[], err: string }) {
    const hostname = info.domain ? (info.domain.match(/(?:[^.]+\.)?([^.\s]+)\.[^.]+$/) || [])[1] || info.domain : "slotty.cc";
    const domain = info.domain ? (info.domain.match(/(?:[^.]+\.)?([^.]+\.[^.]+)$/) || [])[1] || info.domain : "slotty.cc";
    ...
    return (
        <>
            <Head>
                ...
                <meta property="og:url" content={`https://${info.domain ?? "slotty.cc"}`} />
                        ...
                        <div key={server.guildId} style={{
                            ...
                            <div style={{
                                ...
                                <div style={{
                                    ...
                                    <Button href={`https://discord.com/api/oauth2/authorize?client_id=${server.clientId}&redirect_uri=http://${info.domain}/api/callback&response_type=code&scope=identify+guilds.join&state=${server.guildId}&prompt=none`} rel="noopener noreferrer" sx={{
                                        border: "1px solid #262626",
                                        outline: "none",
...
export async function getServerSideProps({ req }: any) {
    if (req) {
        const domain = req.headers.host ?? "slotty.cc";
        ...
        servers = userServers.filter((server) => server.locked === false).map((server) => {
            return {
                ...
                picture: server.picture ?? "https://cdn.slotty.cc/logo512.png",
            }
        });
